...
<Note>
  **Note**: ES modules are a standard for modularizing JavaScript code. They
  became available in ES2015 and are supported by all modern browsers. You can
  read more about them
  [here](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Modules).
</Note>
...
- _Dependencies_. Vite pre-bundles these using [esbuild](https://esbuild.github.io/) (a Go-written bundler 10-100x times faster than JavaScript-written ones)
- _Source code_. Vite serves it over native ESM, this means that the browser does a part of the job of a classic bundler. This way Vite only needs to transform and serve source code when the browser requests it.
...
- _CSS code splitting_: CSS used by modules in async chunks is extracted and then a separate file is generated for it. This way, the CSS is loaded before the async chunk avoiding FUOC (flash of unstyled content).
- _Preload Directives Generation_: generating `<link rel="modulepreload">` directives for entry chunks and their direct imports improves the loading performance of the app by instructing the browser to load the modules before they are requested.
- _Async Chunk Loading Optimization_: Rollup often generates "common" chunks shared by 2 or more other chunks which, in combination with dynamic imports, form the following scenario: <div className="w-full flex justify-center"><Image src='/images/async_chunk_loading.svg' alt='async chunk loading source:"vitejs.org"' width={685} height={246} style={{width: '90%', height: 'auto'}} /></div> So, in non-optimized scenarios, the browser will request and parse chunk A before knowing that it also needs chunk C: `Entry ---> A ---> C`. But what Vite does in this case is transforming the dynamic imports that split code in a process where, when A is fetched, C is also fetched simultaneously: `Entry ---> (A + C)`
